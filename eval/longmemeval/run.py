...
# eval/ was pulled out of backend/; add both the project root (so `eval` is a
# package) and backend/ (so `config`/`memory`/`agents` resolve) to sys.path.
...
# Backend imports (config/memory/agents now resolvable via the shim above)
...
async def reset_memory() -> None:
    ...
    try:
        ...
        # fact_embeddings table may not exist on fresh DBs; facts delete is the
        # important one. Autocommit mode means commit() is a no-op but harmless.
        pass
